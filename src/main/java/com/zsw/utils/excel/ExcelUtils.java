...
import static com.zsw.utils.excel.CommonsUtils.isBlank;
import static com.zsw.utils.excel.CommonsUtils.setDataList;
import static com.zsw.utils.excel.CommonsUtils.setDataListNoStyle;
import static com.zsw.utils.excel.CommonsUtils.strToDateFormat;
import static com.zsw.utils.excel.CommonsUtils.verificationDate;
import static com.zsw.utils.excel.CommonsUtils.writeAndColse;
...
public class ExcelUtils {
    ...
    /**
     * web 响应（response） Excel导出：有样式（行、列、单元格样式）、自适应列宽 功能描述: excel 数据导出、导出模板 更新日志:
     * 1.response.reset();注释掉reset，否在会出现跨域错误。[2018-05-18]
     * 2.新增导出多个单元。[2018-08-08] 3.poi官方建议大数据量解决方案：SXSSFWorkbook。[2018-08-08]
     * 4.自定义下拉列表：对每个单元格自定义下拉列表。[2018-08-08] 5.数据遍历方式换成数组(效率较高)。[2018-08-08]
     * 6.可提供模板下载。[2018-08-08] 7.每个表格的大标题[2018-09-14]
     * 8.自定义列宽：对每个单元格自定义列宽[2018-09-18] 9.自定义样式：对每个单元格自定义样式[2018-10-22]
     * 10.自定义单元格合并：对每个单元格合并[2018-10-22] 11.固定表头[2018-10-23]
     * 12.自定义样式：单元格自定义某一列或者某一行样式[2018-11-12] 13.忽略边框(默认是有边框)[2018-11-15]
     * 14.函数式编程换成面向对象编程[2018-12-06-5]
     * 15.单表百万数据量导出时样式设置过多，导致速度慢（行、列、单元格样式暂时控制10万行、超过无样式）[2019-01-30] 版 本:
     * 1.apache poi 3.17 2.apache poi-ooxml 3.17
     *
     * @return
     */
    public Boolean exportForExcelsOptimize() {
        ...
        try {
            // 设置数据
            setDataList(sxssfWorkbook, sxssfRow, dataLists, notBorderMap, regionMap, mapColumnWidth, styles, paneMap,
    ...
    public Boolean exportForExcelsNoStyle() {
        ...
        SXSSFRow sxssfRow = null;
        try {
            setDataListNoStyle(sxssfWorkbook, sxssfRow, dataLists, notBorderMap, regionMap, mapColumnWidth, paneMap,
                    sheetName, labelName, dropDownMap);
            setIo(sxssfWorkbook, outputStream, fileName, sheetName, response);
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * <p>
     * 更新日志: 1.response.reset();注释掉reset，否在会出现跨域错误。[2018-05-18]
     * 2.新增导出多个单元。[2018-08-08] 3.poi官方建议大数据量解决方案：SXSSFWorkbook。[2018-08-08]
     * 4.自定义下拉列表：对每个单元格自定义下拉列表。[2018-08-08] 5.数据遍历方式换成数组(效率较高)。[2018-08-08]
     * 6.可提供模板下载。[2018-08-08] 7.每个表格的大标题[2018-09-14]
     * 8.自定义列宽：对每个单元格自定义列宽[2018-09-18] 9.自定义样式：对每个单元格自定义样式[2018-10-22]
     ...
     */
    @Deprecated
    public static Boolean exportForExcel(HttpServletResponse response, List<List<String[]>> dataLists,
                                         HashMap notBorderMap, HashMap regionMap, HashMap columnMap, HashMap styles,
                                         HashMap paneMap, String fileName, String[] sheetName, String[] labelName,
                                         HashMap rowStyles, HashMap columnStyles, HashMap dropDownMap) {
        long startTime = System.currentTimeMillis();
        log.info("=== ===  === :Excel tool class export start run!");
        SXSSFWorkbook sxssfWorkbook = new SXSSFWorkbook(1000);
        OutputStream outputStream = null;
        ...
        try {
            // 设置数据
            setDataList(sxssfWorkbook, sxssfRow, dataLists, notBorderMap, regionMap, columnMap, styles, paneMap,
                    sheetName, labelName, rowStyles, columnStyles, dropDownMap);
            // io 响应
            setIo(sxssfWorkbook, outputStream, fileName, sheetName, response);
        } catch (Exception e) {
     ...
     * 功能描述: 1.excel 模板数据导入。
     * <p>
     * 更新日志: 1.共用获取Excel表格数据。 2.多单元数据获取。 3.多单元从第几行开始获取数据[2018-09-20]
     ...
     * @param indexMap 多单元从第几行开始获取数据，默认从第二行开始获取（可为空，如 hashMapIndex.put(1,3);
     *            第一个表格从第三行开始获取）
     * @param continueRowMap 多单元根据那些列为空来忽略行数据（可为空，如 mapContinueRow.put(1,new
     *            Integer[]{1, 3}); 第一个表格从1、3列为空就忽略）
     * @return
     */
    public static List<List<LinkedHashMap<String, String>>> importForExcelData(Workbook book, String[] sheetName,
        ...
        try {
            ...
            log.info("=== ===  === :Excel tool class export run time:" + (System.currentTimeMillis() - startTime)
                    + " ms!");
     ...
     * @throws Exception
     */
    private static void setIo(SXSSFWorkbook sxssfWorkbook, OutputStream outputStream, String fileName,
        ...
        try {
            if (response != null) {
                ...
                response.setHeader("Content-disposition", "attachment; filename="
                        + URLEncoder.encode(fileName == null ? sheetName[0] : fileName, "utf8") + ".xlsx");
        ...
        if (cell != null) {
            ...
            switch (cellType) {
                case NUMERIC:
                    ...
                    break;
                case STRING:
                    if (cell.getStringCellValue().trim().length() >= DATE_LENGTH
                            && verificationDate(cell.getStringCellValue(), ExcelUtils.initialization().dateFormatStr)) {
                        val = strToDateFormat(cell.getStringCellValue(), ExcelUtils.initialization().dateFormatStr,
                                ExcelUtils.initialization().expectDateFormatStr);
                    } else {
