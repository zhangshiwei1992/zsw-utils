...
public class CommonsUtils {
    ...
    public static void setDataList(SXSSFWorkbook wb, SXSSFRow sxssfRow, List<List<String[]>> dataLists,
                                   HashMap notBorderMap, HashMap regionMap, HashMap columnMap, HashMap styles,
                                   HashMap paneMap, String[] sheetName, String[] labelName, HashMap rowStyles,
                                   HashMap columnStyles, HashMap dropDownMap)
            throws Exception {
        if (dataLists == null) {
            log.debug(
                    "=== ===  === :Andyczy ExcelUtils Exception Message：Export data(type:List<List<String[]>>) cannot be empty!");
        }
        if (sheetName == null) {
            log.debug(
                    "=== ===  === :Andyczy ExcelUtils Exception Message：Export sheet(type:String[]) name cannot be empty!");
        }
        int k = 0;
                ...
                for (int j = 0; j < listRow.get(i).length; j++) {
                    ...
                    try {
                        ...
                        if (styles != null && i <= MAXSYTLE) {
                            ...
                            setExcelStyles(cell, cell_style, wb, sxssfRow, (List<List<Object[]>>) styles.get(k + 1), j,
                                    i);
                        }
                    } catch (Exception e) {
                        log.debug(
                                "=== ===  === :Andyczy ExcelUtils Exception Message：The maximum number of cell styles was exceeded. You can define up to 4000 styles!");
                    }
                }
                jRow++;
            }
     ...
     * @param height 行高
     */
    public static void setExcelStyles(Cell cell, CellStyle cellStyle, SXSSFWorkbook wb, SXSSFRow sxssfRow,
                                      Integer fontSize, Boolean bold, Boolean center, Boolean isBorder,
                                      Boolean leftBoolean, Boolean rightBoolean, Integer fontColor, Integer height) {
    ...
    public static void setExcelRowStyles(Cell cell, CellStyle cellStyle, SXSSFWorkbook wb, SXSSFRow sxssfRow,
                                         List<Object[]> rowstyleList, int rowIndex) {
        if (rowstyleList != null && rowstyleList.size() > 0) {
            ...
            for (int i = 0; i < rowstyle.length; i++) {
                if (rowIndex == rowstyle[i]) {
                    ...
                    setExcelStyles(cell, cellStyle, wb, sxssfRow, fontSize, Boolean.valueOf(bool[3]),
                            Boolean.valueOf(bool[0]), Boolean.valueOf(bool[4]), Boolean.valueOf(bool[2]),
                            Boolean.valueOf(bool[1]), fontColor, height);
                }
            }
        }
    }
     ...
     * @throws Exception
     */
    public static void setDataListNoStyle(SXSSFWorkbook wb, SXSSFRow sxssfRow, List<List<String[]>> dataLists,
                                          HashMap notBorderMap, HashMap regionMap, HashMap columnMap, HashMap paneMap,
                                          String[] sheetName, String[] labelName, HashMap dropDownMap)
            throws Exception {
        if (dataLists == null) {
            log.debug(
                    "=== ===  === :Andyczy ExcelUtils Exception Message：Export data(type:List<List<String[]>>) cannot be empty!");
    ...
    /**
     * 功能描述：所有自定义单元格样式 使用的方法：是否居中？，是否右对齐？，是否左对齐？， 是否加粗？，是否有边框？ —— 颜色、字体、行高？
     * HashMap cellStyles = new HashMap(); List< List<Object[]>> list = new
     * ArrayList<>(); List<Object[]> objectsList = new ArrayList<>();
     * List<Object[]> objectsListTwo = new ArrayList<>(); objectsList.add(new
     * Boolean[]{true, false, false, false, true}); //1、样式一（必须放第一）
     * objectsList.add(new Integer[]{10, 12}); //1、颜色值 、字体大小、行高（必须放第二）
     * <p>
     * objectsListTwo.add(new Boolean[]{false, false, false, true, true});
     * //2、样式二（必须放第一） objectsListTwo.add(new Integer[]{10, 12,null}); //2、颜色值
     * 、字体大小、行高（必须放第二）
     * <p>
     * objectsList.add(new Integer[]{5, 1}); //1、第五行第一列 objectsList.add(new
     * Integer[]{6, 1}); //1、第六行第一列
     * <p>
     ...
     */
    public static void setExcelStyles(Cell cell, CellStyle cellStyle, SXSSFWorkbook wb, SXSSFRow sxssfRow,
                                      List<List<Object[]>> styles, int cellIndex, int rowIndex) {
        if (styles != null) {
    ...
    /**
     * 功能描述: 锁定行（固定表头） 参数说明： HashMap setPaneMap = new HashMap();
     * //第一个表格、第三行开始固定表头 setPaneMap.put(1,3);
     *
     * @param sxssfSheet
     * @param row
     */
    ...
    /**
     * 功能描述: 自定义列宽 参数说明： HashMap<Integer, HashMap<Integer, Integer>> columnMap =
     * new HashMap<>(); HashMap<Integer, Integer> mapColumn = new HashMap<>();
     * //第一列、宽度为 3[3的大小就是两个12号字体刚刚好的列宽]（注意：excel从零行开始数） mapColumn.put(0, 3);
     * mapColumn.put(1, 20); mapColumn.put(2, 15); //第一个单元格列宽 columnMap.put(1,
     * mapColumn);
     *
     * @param sxssfSheet
     * @param map
    ...
    /**
     * 功能描述: excel 合并单元格 参数说明： List<List<Integer[]>> regionMap = new
     * ArrayList<>(); List<Integer[]> regionList = new ArrayList<>();
     * //代表起始行号，终止行号， 起始列号，终止列号进行合并。（注意：excel从零行开始数） regionList.add(new
     * Integer[]{1, 1, 0, 10}); regionList.add(new Integer[]{2, 3, 1, 1});
     * //第一个表格设置。 regionMap.put(1, regionList);
     *
     * @param sheet
     * @param rowColList
     */
    ...
    /**
     * 功能描述:下拉列表 参数说明： HashMap dropDownMap = new HashMap(); List<String[]>
     * dropList = new ArrayList<>(); //必须放第一：设置下拉列表的列（excel从零行开始数） String[]
     * sheetDropData = new String[]{"1", "2", "4"}; //下拉的值放在 sheetDropData 后面。
     * String[] sex = {"男,女"}; dropList.add(sheetDropData); dropList.add(sex);
     * //第一个表格设置。 dropDownMap.put(1,dropList);
     *
     * @param sheet
    ...
    public static void setDataValidation(SXSSFSheet sheet, List<String[]> dropDownListData, int dataListSize) {
        if (dropDownListData.size() > 0) {
            for (int col = 0; col < dropDownListData.get(0).length; col++) {
                ...
                setDataValidation(sheet, dropDownListData.get(col + 1), 1, dataListSize < 100 ? 500 : dataListSize,
                        colv, colv);
            }
        }
    }
     ...
     * @param lastCol
     */
    public static void setDataValidation(SXSSFSheet xssfWsheet, String[] list, Integer firstRow, Integer lastRow,
